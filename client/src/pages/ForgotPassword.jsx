      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-card border border-blue-20 p-8 animate-fade-in-up">
          <div className="text-center mb-6">
            ...
            <h2 className="text-2xl font-outfit font-bold text-heading">Forgot Password</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4" id="forgot-password-form">
            <div>
              ...
              <input type="email" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} required id="forgot-email-input"
            ...
            <button type="submit" disabled={loading} id="forgot-submit-btn"
              ...
              {loading ? <><div className="w-5 h-5 rounded-full border-2 border-white/30 border-t-white animate-spin" />Sending...</> : "Send Reset Link"}
            </button>
          </form>

          <p className="text-center text-sm text-muted mt-6">
            <Link to="/login" className="text-primary font-medium hover:text-primary-dark transition-colors">← Back to Login</Link>
          </p>
        </div>
      </div>
    </div>
