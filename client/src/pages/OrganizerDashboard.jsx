...
export default function OrganizerDashboard() {
  ...
  const [newEvent, setNewEvent] = useState({ name: "", date: "", location: "", capacity: "" });
  ...
  const handleCreateEvent = async (e) => {
    ...
    try {
      ...
      setShowModal(false);
      setNewEvent({ name: "", date: "", location: "", capacity: "" });
    } catch (err) {
      console.error(err);
    }
  };
      ...
      <aside className="bg-blue-900 text-white w-64 flex flex-col">
        ...
        <nav className="flex-1">
          {sidebarLinks.map((link) => (
            <div key={link.name} className="flex items-center p-4 hover:bg-blue-700 cursor-pointer transition">
      ...
      <main className="flex-1 bg-gray-100 p-6 overflow-auto">
        ...
        <div className="flex justify-between items-center mb-6 bg-white p-4 rounded-xl shadow-sm">
          <div>
            ...
            <p className="text-gray-600 text-sm mt-1">Manage your events and track performance</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          {metrics.map((m) => (
            <div key={m.title} className="bg-white shadow rounded p-4 hover:shadow-lg transition">
        ...
        <div className="bg-white shadow rounded p-4 mb-6">
          ...
          <table className="min-w-full text-left text-sm text-gray-600">
            ...
            <tbody>
              {events.map((event) => (
                <tr key={event.id || event.name} className="border-b hover:bg-gray-50">
